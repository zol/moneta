...
public class DepthTimedWriter extends TimerTask {
  static Logger logger = Logger.getLogger(DepthTimedWriter.class);
  
  private Hashtable<Integer, DepthTableData> ids = new Hashtable();
  Timer timer;
  Connection conn = null;
  
  public DepthTimedWriter() 
  {
  ...
  public void write() {
    ...
    try {
      ...
      logger.info("Wrote DepthTableEvents to DB at: " + dateString);
  ...
  public void onDepthTableEvent(DepthTableEvent evt) {
    ids.put(new Integer(evt.getId()), evt.getDepthTable());
